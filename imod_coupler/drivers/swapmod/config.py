...
class Coupling(BaseModel):
    ...
    @validator("mf6_swap_sprinkling_map")
    def validate_mf6_swap_sprinkling_map(
        cls, mf6_swap_sprinkling_map: Optional[FilePath], values: Any
    ) -> Optional[FilePath]:
        if mf6_swap_sprinkling_map is not None:
            return mf6_swap_sprinkling_map.resolve()
        elif values.get("enable_sprinkling"):
            raise ValueError(
                "If `enable_sprinkling` is True, then `mf6_swap_sprinkling_map` needs to be set."
            )
        return mf6_swap_sprinkling_map
